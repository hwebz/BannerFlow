...
function updateWaves() {    
    var numChild = parseInt(BannerFlow.settings.numberChild);
    var lineColor = BannerFlow.settings.lineColor;
    var speed = BannerFlow.settings.speed;
    var amplitude = BannerFlow.settings.amplitude;
    var lineWidth = BannerFlow.settings.lineWidth;
    var waveTypes = parseInt(BannerFlow.settings.waveTypes);
    var orgRGB = lineColor.substr(lineColor.lastIndexOf("("), lineColor.lastIndexOf(","));
    var temp = orgRGB.substr(0, orgRGB.lastIndexOf(","));
    ...
    switch (waveTypes) {
        case 1:
            var waves = [];
            var timeModifierArr = [4, 2, 1, 0.5, 0.25];
            var ampArr = [-25, -50, -100, -200, -400];
            var waveLengthArr = [25, 50, 100, 200, 400];
            var amp, waveLength, timeModifier, randomRatio;
            
            for (var i = 0; i < numChild; i++) {
                randomRatio = 4/getRandomArbitrary(0.25, 4);
                timeModifier = i < timeModifierArr.length ? timeModifierArr[i] : randomRatio;
                amp = i < ampArr.length ? ampArr[i]*amplitude/5 : -400/randomRatio*amplitude/5;
                waveLength = i < waveLengthArr.length ? waveLengthArr[i] : 400/randomRatio;
                waves.push({
            ...
            SinWavesInit(waves, speed, '70%', function() {
                var gradient = this.ctx.createLinearGradient(0, 0, this.width, 0);
                gradient.addColorStop(0,"rgba" + temp + ",0.2)");
                gradient.addColorStop(0.5, "rgba" + temp + ",0.7)");
                gradient.addColorStop(1, "rgba" + temp + ",0.2)");
        ...
        case 2:
            var waves = [];
            var timeModifier = 1;
            var ampArr = [150, 150, -150, -100, 100, -150, -100, -50, 125, 75, 25, -25, -75, -125, -150];
            var waveLengthArr = [200, 100, 50, 100, 150, 125, 75, 25];
            var amp, waveLength;
            for (var i = 0; i < numChild; i++) {
                amp = ampArr[ampArr.length > numChild ? i : i % ampArr.length]*amplitude/5;
                waveLength = waveLengthArr[waveLengthArr.length > numChild ? i : i % waveLengthArr.length];
                waves.push({
                  timeModifier: timeModifier,
                  lineWidth: (lineWidth - 1) + (i == 0 ? 0.5 : i),
                  amplitude: amp,
                  wavelength: waveLength,
                  segmentLength: 1
                });
            }
            SinWavesInit(waves, speed, '100%', function() {
                var gradient = this.ctx.createLinearGradient(0, 0, this.width, 0);
                gradient.addColorStop(0,"rgba" + temp + ",0)");
                gradient.addColorStop(0.5,"rgba" + temp + ",0.7)");
                gradient.addColorStop(1,"rgba" + temp + ",0)");
                var index = -1;
                var length = this.waves.length;
                  while(++index < length){
                  this.waves[index].strokeStyle = gradient;
       ...
       case 3:
            var waves = [];
            var timeModifier = 4;
            var ampArr = [-100, -50, 50, 100, 75, 125, -125];
            var waveLengthArr = [25, 20, 50, 75, 40, 15, 35, 65];
            var amp, waveLength, segmentLength;
            for (var i = 0; i < numChild; i++) {
                amp = ampArr[ampArr.length > numChild ? i : i % ampArr.length]*amplitude/5;
                waveLength = waveLengthArr[waveLengthArr.length > numChild ? i : i % waveLengthArr.length];
                waves.push({
            ...
            SinWavesInit(waves, speed, '80%', function() {
                var gradient = this.ctx.createLinearGradient(0, 0, this.width, 0);
...
function SinWavesInit(waves, speed, wavesWidth, resizeEventCallback) {
    ...
    waves = new SineWaves({
      ...
      resizeEvent: resizeEventCallback
    });
}
var c;
var isDone = true;
